...
# Desarrollado por Daniel Diaz Llanos
...
for x in range(0, numero_lineas_nomina):
	linea_validada = validar_linea(lineas_archivo_nomina[x], x1)
	
	if(linea_validada[0] == False):
		terminar_programa("Error en archivo de nomina!")

	guardar_log("Procesando linea "  str(x1));
...
for z in range(0, numero_lineas_nomina):
	...
	guardar_log("Calculando liquidacion para empleado "  str(z1));	
...
contenido_linea = ("total nomina",(total_ingenierostotal_tecnicostotal_operariostotal_secretarios),"Ingenier@s", total_ingenieros, "Tecnic@s", total_tecnicos, "Operari@s", total_operarios, "Secretari@s", total_secretarios)
